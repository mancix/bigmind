...
/** Subscribe to newly created conflict records (used by the web toast UI). */
...
/**
 * Conflict repository shared by the web app and the mobile app.
 *
 * Persists detected sync conflicts and implements every resolution strategy
 * (keep_mine / keep_remote / merge_manually / restore / delete_mine) over the
 * {@link StorageAdapter} + outbox abstractions — nothing platform-specific.
 * The sync engine uses it as its {@link SyncOutbox}-backed `ConflictSink`.
 */
export class ConflictRepository {
  ...
  async listOpen(): Promise<ConflictRecord[]> {
    const conflicts = await this.storage.conflicts
      .where('status')
      .equals('open')
      .toArray();
  ...
  async listDismissed(): Promise<ConflictRecord[]> {
    return this.storage.conflicts.where('status').equals('dismissed').toArray();
  }

  async find(id: string): Promise<ConflictRecord | undefined> {
    return this.storage.conflicts.get(id);
  }

  async countOpen(): Promise<number> {
    return this.storage.conflicts.where('status').equals('open').count();
  }
  ...
  async create(input: CreateConflictInput): Promise<string> {
    ...
    if (existing) {
      ...
      await this.storage.conflicts.put(merged);
    ...
    await this.storage.conflicts.add(conflict);
  ...
  async dismiss(id: string): Promise<void> {
    ...
    await this.storage.conflicts.update(id, {
      status: 'dismissed',
      resolvedAt,
      resolution: 'dismiss',
    });
  ...
  async resolveMergeManually(id: string, mergedEntity: unknown): Promise<void> {
    ...
    const existing = await this.storage.notes.get(conflict.entityId);
    ...
    await this.storage.notes.put(mergedNote);
  ...
  async resolveRestore(id: string): Promise<void> {
    ...
    await this.storage.notes.put(restored);
    await this.requeueOperationForEntity(
      'note',
      conflict.entityId,
      conflict.remoteVersion,
  ...
  async resolveDeleteMine(id: string): Promise<void> {
    ...
    if (conflict.entityType === 'note') {
      ...
      await this.storage.noteLinks.delete(conflict.entityId);
  ...
  private async requireConflict(id: string): Promise<ConflictRecord> {
    const conflict = await this.storage.conflicts.get(id);
  ...
  private async markResolved(
    ...
    await this.storage.conflicts.put({
      ...conflict,
      status: 'resolved',
      resolvedAt,
      resolution,
  ...
  private async clearEntityConflictFlag(conflictId: string): Promise<void> {
    const conflict = await this.storage.conflicts.get(conflictId);
    if (!conflict) return;

    if (conflict.entityType === 'note') {
      const note = await this.storage.notes.get(conflict.entityId);
      if (note && note.syncStatus === 'conflict') {
        await this.storage.notes.update(conflict.entityId, {
          conflict: undefined,
        });
      }
    } else if (conflict.entityType === 'category') {
      const category = await this.storage.categories.get(conflict.entityId);
      if (category && category.syncStatus === 'conflict') {
        await this.storage.categories.update(conflict.entityId, {
          conflict: undefined,
        });
      }
    }
  ...
  private async requeueEntityAsPendingUpdate(
    ...
    const record =
      entityType === 'note'
        ? await this.storage.notes.get(entityId)
        : await this.storage.categories.get(entityId);
    ...
    await (entityType === 'note'
      ? this.storage.notes.put(pendingRecord as NoteRecord)
      : this.storage.categories.put(pendingRecord as CategoryRecord));
    await this.requeueOperationForEntity(
      entityType,
      entityId,
      baseVersion,
  ...
  private async requeueLinkOperation(
    ...
    const link = await this.storage.noteLinks.get(conflict.entityId);
    if (!link) return;
  ...
  private async applyRemoteNoteSnapshot(
    ...
    await this.storage.notes.put(remoteRecord);
    await this.outbox.removeMany(
  ...
  private async applyRemoteCategorySnapshot(
    ...
    if (remoteRecord.parentId) {
      const categories = (await this.storage.categories.toArray()).filter(
    ...
    await this.storage.categories.put(remoteRecord);
    await this.outbox.removeMany(
  ...
  private async applyRemoteLinkSnapshot(
    ...
    await this.storage.noteLinks.put(remoteRecord);
    await this.outbox.removeMany(
      (await this.outbox.listForEntity(conflict.entityId, 'link')).map(
        (operation) => operation.id,
      ),
  ...
  private async acceptRemoteNoteDeletion(
    ...
    if (existing) {
      ...
      await this.storage.notes.put(deleted);
    }
  ...
  private async acceptRemoteCategoryDeletion(
    ...
    const existing = await this.storage.categories.get(conflict.entityId);
    const remoteChangedAt =
      conflict.remoteSnapshot.changedAt ?? new Date().toISOString();
    ...
    if (existing) {
      ...
      await this.storage.categories.put(deleted);
    }
