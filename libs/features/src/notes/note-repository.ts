...
/**
 * Note repository shared by the web app and the mobile app.
...
export class NoteRepository {
  constructor(
    ...
    private readonly links: LinkRepository = new LinkRepository(
      storage,
      outbox,
    ),
  ) {}
  ...
  async list(query: NoteListQuery = {}): Promise<NoteRecord[]> {
    ...
    const aliases = search ? await this.storage.noteAliases.toArray() : [];
  ...
  async findById(id: string): Promise<NoteRecord | undefined> {
    const note = await this.storage.notes.get(id);
  ...
  async create(input: CreateNoteInput = {}): Promise<string> {
    ...
    await this.outbox.transactionWithNoteGraph(async () => {
      await this.storage.notes.add(note);
  ...
  async update(id: string, changes: UpdateNoteInput): Promise<void> {
    await this.outbox.transactionWithNoteGraph(async () => {
      const existingNote = await this.storage.notes.get(id);
      ...
      await this.storage.notes.put(updatedNote);
      ...
      if (pendingCreate) {
        await this.outbox.save(this.resetOperation(pendingCreate, updatedNote));
        return;
      }
      ...
      if (pendingUpdate) {
        await this.outbox.save(this.resetOperation(pendingUpdate, updatedNote));
        return;
      }
  ...
  async delete(id: string): Promise<void> {
    await this.outbox.transactionWithNoteGraph(async () => {
      ...
      await this.storage.notes.put(deletedNote);
  ...
  async count(): Promise<number> {
    return this.storage.notes.filter((note) => !isNoteDeleted(note)).count();
  }
