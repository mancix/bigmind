...
import type { RemoteChange } from '@bigmind/domain/sync';
...
/**
 * Conflict classification and snapshot building, shared by the sync engine
 * on every platform (web and mobile use this exact implementation).
 */
export class ConflictService {
  determineConflictType<TEntity>(
    ...
    if (entityType === 'note') {
      ...
      const remotePayload = remoteChange.payload as
        Partial<NoteRecord> | undefined;
    ...
    if (entityType === 'category') {
      ...
      const remotePayload = remoteChange.payload as
        Partial<CategoryRecord> | undefined;

      if (!local || !remotePayload) return 'generic';

      const parentChanged =
        'parentId' in remotePayload &&
        remotePayload.parentId !== local.parentId;
      const nameChanged =
  ...
  canAutoResolve(
    conflict: Pick<
      Conflict,
      'conflictType' | 'localSnapshot' | 'remoteSnapshot'
    >,
  ): boolean {
    ...
    const localEntity = (conflict.localSnapshot as ConflictSnapshot).entity as
      NoteRecord | CategoryRecord | NoteLinkRecord | undefined;
    const remoteEntity = (conflict.remoteSnapshot as ConflictSnapshot)
      .entity as NoteRecord | CategoryRecord | NoteLinkRecord | undefined;
