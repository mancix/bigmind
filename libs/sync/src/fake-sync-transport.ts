...
/**
 * In-memory sync transport used by tests and by the fake-sync development
 * mode. Platform independent.
 */
export class FakeSyncTransport implements SyncTransport {
  ...
  private readonly processedOperations = new Map<string, PushOperationResult>();
  ...
  private process(operation: SyncOperation): PushOperationResult {
    ...
    if (
      !current ||
      current.deleted ||
      operation.baseVersion !== current.version
    ) {
      return this.conflict(operation, current);
    }
  ...
  private accept(
    ...
    const versionedRecord: SyncRecord = {
      ...
      version,
      syncStatus: 'synced',
      ...(deleted && 'deletedAt' in record
        ? { deletedAt: record.deletedAt ?? changedAt }
        : {}),
    };
