/**
 * Canonical Markdown → HTML renderer.
 *
 * Byte-for-byte port of the web app's legacy `render-markdown.ts` so existing
 * surfaces (category descriptions, previews) render identically while the
 * implementation lives in ONE shared place. The React Native preview uses the
 * structured tokenizer (`parse.ts`/`inline.ts`) instead; this function serves
 * the web and tests as the divergence guard.
 */
