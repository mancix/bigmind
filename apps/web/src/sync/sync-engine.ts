...
import {
  ConflictService,
  conflictService,
} from '../features/conflicts/conflict-service';
import { OutboxRepository, outboxRepository } from './outbox-repository';
import {
  SyncStateRepository,
  syncStateRepository,
...
export class SyncEngine {
  ...
  constructor(
    ...
    private readonly syncState: SyncStateRepository = syncStateRepository,
    private readonly conflicts: ConflictRepository = conflictRepository,
    private readonly conflictSvc: ConflictService = conflictService,
  ...
  private async execute(): Promise<void> {
    ...
    try {
      await this.outbox.resetStaleProcessing(this.now());
      processing = await this.outbox.listPending(this.now());

      if (processing.length > 0) {
        await this.outbox.markProcessing(
          processing.map((operation) => operation.id),
          this.now(),
        );
        const results = await this.transport.push(processing);
        await this.processPushResults(processing, results);
      }

      const cursor = await this.syncState.getCursor();
      const pullResult = await this.transport.pull(cursor);

      await this.applyPullResult(pullResult.changes, pullResult.cursor);
      this.setStatus('idle');
    } catch (error) {
      if (authStore.getState() === 'auth_required') {
  ...
  private async acceptOperation(
    ...
    await this.outbox.transactionWithEntities(async () => {
      const currentOperation = await this.outbox.get(operation.id);

      if (!currentOperation) {
        return;
      }

      const table = operation.entityType === 'note'
        ? db.notes
        : operation.entityType === 'category'
          ...
          : operation.entityType === 'reminder'
            ...
            : operation.entityType === 'notification'
              ? db.notifications
              : db.noteLinks;
      const record = await table.get(operation.entityId) as SyncRecord | undefined;
      const otherOperations = (await this.outbox.listForEntity(
        operation.entityId,
      ...
      await this.outbox.markCompleted(operation.id);
    });
  }
  ...
  private async markConflict(
    ...
    if (operation.entityType === 'link' || operation.entityType === 'todo_item' || operation.entityType === 'reminder' || operation.entityType === 'notification') {
      await this.outbox.markFailed(operation.id, result.error);
      return;
    }
    ...
    const markFailed = () => this.outbox.markFailed(operation.id, result.error);

    await this.outbox.transactionWithEntities(async () => {
      const table = entityType === 'note' ? db.notes : db.categories;
  ...
  private async failOperation(
    ...
    const retryCount = await this.outbox.incrementRetryCount(operationId);
    ...
    await this.outbox.markFailed(operationId, error, nextRetryAt);
  }
  ...
  private async failStillProcessing(
    ...
    const error: SyncError = {
      ...
      message: cause instanceof Error ? cause.message : 'Synchronization failed.',
      retryable: true,
    };

    for (const operation of operations) {
      if ((await this.outbox.get(operation.id))?.status === 'processing') {
        await this.failOperation(operation.id, error);
      }
    }
  }

  private async failAuthOperations(
    operations: OutboxRecord[],
  ): Promise<void> {
    const error: SyncError = {
      code: 'auth_required',
    ...
    for (const operation of operations) {
      if ((await this.outbox.get(operation.id))?.status === 'processing') {
        await this.outbox.markFailed(operation.id, error, undefined);
      }
    }
  }
  ...
  private async applyPullResult(
    ...
    await this.outbox.transactionWithEntitiesAndSyncState(async () => {
      ...
      await this.syncState.recordSuccessfulSync(
        cursor,
  ...
  private async applyRemoteChange(change: RemoteChange): Promise<void> {
    const table = change.entityType === 'note'
      ? db.notes
        ...
        : change.entityType === 'todo_item'
          ...
          : change.entityType === 'reminder'
            ...
            : change.entityType === 'notification'
              ? db.notifications
              : db.noteLinks;
    const existing = await table.get(change.entityId) as SyncRecord | undefined;
    const localOperations = (await this.outbox.listForEntity(
      change.entityId,
      change.entityType,
    ))
    ...
    if (change.operation === 'delete') {
      if (change.entityType === 'link') {
        await db.noteLinks.delete(change.entityId);
        return;
      }
      if (change.entityType === 'todo_item') {
        await db.todoItems.delete(change.entityId);
        return;
      }
      if (change.entityType === 'reminder') {
        await db.reminders.delete(change.entityId);
        return;
      }
      if (change.entityType === 'notification') {
        await db.notifications.delete(change.entityId);
        return;
      }
    ...
    if (!existing || change.version >= existing.version) {
      if (change.entityType === 'link') {
        ...
        const target = await db.notes.get(payload.targetNoteId ?? '');
        await db.noteLinks.put({
          ...payload,
          targetTitle: existing && 'targetTitle' in existing
  ...
  const snapshots = this.conflictSvc.buildSnapshots({
    entityType,
    entityId,
    localEntity,
    remoteChange,
