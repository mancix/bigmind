...
import type { OutboxRecord } from '../storage/database';
...
function mockOutbox() {
  ...
  return {
    ...
    transactionWithEntities: vi.fn().mockImplementation((cb: () => Promise<void>) => cb()),
    transactionWithEntitiesAndSyncState: vi.fn().mockImplementation((cb: () => Promise<void>) => cb()),
    transactionWithNotes: vi.fn().mockImplementation((cb: () => Promise<void>) => cb()),
    transactionWithNoteGraph: vi.fn().mockImplementation((cb: () => Promise<void>) => cb()),
    transactionWithCategories: vi.fn().mockImplementation((cb: () => Promise<void>) => cb()),
...
function mockConflictService() {
  return {
    buildSnapshots: vi.fn().mockReturnValue({
...
describe('SyncEngine auth integration', () => {
  ...
  beforeEach(() => {
    ...
    engine = new SyncEngine(
      transport,
      mockOutbox() as any,
      mockSyncState() as any,
      mockConflicts() as any,
      mockConflictService() as any,
      {
  ...
  it('transitions from auth_required to syncing after re-authentication', async () => {
    ...
    authStore.setTokens('new-access', 'new-refresh', { id: 'u1', email: 'a@b.com' });
