...
export class CategoryRepository {
  ...
  async list(): Promise<CategoryRecord[]> {
    return (await db.categories.toArray()).filter(
  ...
  async create(input: CreateCategoryInput): Promise<string> {
    const categoryId = await this.outbox.transactionWithCategories(async () => {
      ...
      await this.outbox.add(this.createOperation('create', category, timestamp));
      return category.id;
    });
  ...
  async delete(id: string): Promise<void> {
    await this.outbox.transactionWithEntities(async () => {
      ...
      if (
        await db.notes.filter(
          (note) => !note.deletedAt && note.categoryId === id,
        ).count()
      ) {
        throw new CategoryRepositoryError(
          'CATEGORY_HAS_NOTES',
      ...
      await db.categories.put(category);
      const reusable = operations.find(({ operation }) => operation === 'delete')
      ...
      if (reusable) {
        ...
        await this.outbox.add(this.createOperation('delete', category, timestamp));
      }
    });
  ...
  private async requireCategory(id: string): Promise<CategoryRecord> {
    ...
    if (!category) {
      throw new CategoryRepositoryError('CATEGORY_NOT_FOUND', 'The category does not exist.');
    }
    return category;
  }
  ...
  private async upsertOperation(
    ...
    const reusable = operations.find(({ operation }) => operation === 'create')
      ?? operations.find(({ operation }) => operation === 'update');
    if (reusable) {
      await this.outbox.save(this.resetOperation(reusable, category));
    } else {
      await this.outbox.add(this.createOperation('update', category, timestamp));
    }
  }
