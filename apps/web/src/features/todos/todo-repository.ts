import { db, type OutboxRecord, type TodoItemRecord } from '../../storage/database';
...
export class TodoRepository {
  ...
  private async findOrCreateListId(noteId: string): Promise<string> {
    ...
    const existing = await db.todoItems
      .where('todoListId')
      .equals(todoListId)
      .filter((item) => !item.deletedAt)
      .first();
  ...
  async update(id: string, text: string): Promise<TodoItemRecord> {
    ...
    await this.outbox.transactionWithTodos(async () => {
      ...
      await db.todoItems.put(updated);
    ...
    requestBackgroundSync();
    return (await db.todoItems.get(id))!;
  }
  ...
  async remove(id: string): Promise<void> {
    ...
    await this.outbox.transactionWithTodos(async () => {
      ...
      await db.todoItems.put(deleted);
  ...
  async reorder(noteId: string, itemId: string, newPosition: number): Promise<TodoItemRecord[]> {
    ...
    await this.outbox.transactionWithTodos(async () => {
      for (let i = 0; i < reordered.length; i++) {
        ...
        if (current.position !== i) {
          ...
          await db.todoItems.put(current);
          ...
          if (pendingOp) {
            ...
            await this.outbox.add(this.createOperation('update', current, this.now()));
          }
        }
      }
    });
  ...
  private async updateInternal(id: string, changes: Partial<TodoItemRecord>): Promise<TodoItemRecord> {
    const timestamp = this.now();
