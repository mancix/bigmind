...
const links = new LinkRepository(outbox);
const notes = new NoteRepository(outbox, links);
...
describe('link repository', () => {
  it('builds outgoing links and backlinks completely offline', async () => {
    ...
    expect((await links.getOutgoingLinks(ownershipId)).map(({ id }) => id))
      .toEqual([rustId]);
    expect((await links.getBacklinks(rustId)).map(({ id }) => id))
      .toEqual([ownershipId]);
    expect((await outbox.listPending()).some(({ entityType }) => entityType === 'link'))
      .toBe(true);
  });
  ...
  it('keeps unresolved links local and resolves them when a note is created', async () => {
    ...
    expect((await links.getOutgoingLinks(sourceId)).map(({ id }) => id))
      .toEqual([lifetimeId]);
  });
  ...
  it('preserves links through rename and searches old aliases', async () => {
    ...
    expect(await links.aliasesForNote(rustId)).toMatchObject([{ alias: 'Rust' }]);
  ...
  it('updates links after content edits and removes them with deleted notes', async () => {
    ...
    const sourceId = await notes.create({ title: 'Ownership', content: '[[Rust]]' });
