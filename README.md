...
libs/
  ...
  markdown/     Single source of truth for Markdown: shared AST parser,
                 wiki-link references + backlink index, previews,
                 search preparation, renderer abstraction
                 (see docs/markdown-architecture.md)
docker/
...
- **Markdown editing (native editor, Option B, shipped)** — the note detail embeds `MarkdownEditView` (`apps/mobile/src/components/`): raw multiline `TextInput` as the source of truth (no rich-text model), a **formatting toolbar** (heading, bold, italic, inline code, bullet/ordered/checklist lists, code block, quote, link, `[[` wiki link, preview) built on **pure, cursor-safe string transforms** from `@bigmind/markdown` (`format.ts` — every action returns `{ text, start, end }` so the selection stays anchored), `[[` wiki-link suggestions from the shared ranking helper, an edit ⇄ **preview** toggle rendered by the shared tokenizer (`MarkdownText`), and **debounced autosave** (700 ms after inactivity) that persists through the shared `NoteRepository` — updating wiki links/backlinks and coalescing a sync (outbox) operation — fully offline. `TODO_LIST` notes switch to a native `TodoListView` over the shared `TodoRepository`. See [docs/mobile-editor.md](docs/mobile-editor.md), [docs/markdown-architecture.md](docs/markdown-architecture.md).
...
- **Reminders** — the Reminders tab is a native stack (`RemindersNavigator`): `RemindersListScreen` renders the **Agenda** (Today / Tomorrow / Upcoming / Completed via the pure `buildAgendaReminders` helpers, offline title + description search, loading/empty/offline states, row completion toggling, linked-note chips, confirmed deletion) → `ReminderDetailScreen` (title, description, due date, status, linked note navigation, edit/delete) → `ReminderFormScreen` (create/edit: title, description, due date via the native date+time picker, optional linked note picker, and completion status in edit mode). All mutations go through the **shared `RemindersRepository`** — offline-first with outbox coalescing and workspace scoping, identical to the web Agenda page — and automatically schedule **native local notifications** through the platform notification scheduler (see [docs/mobile-notifications.md](docs/mobile-notifications.md)). See [docs/mobile-reminders.md](docs/mobile-reminders.md) for the full mobile reminder architecture, agenda rules, and sync behavior.
- All editing goes through the **shared repositories** (`@bigmind/features`): `NoteRepository`, `CategoryRepository`, `LinkRepository`, `TodoRepository`, plus `RemindersRepository`, `NotificationsRepository`, and `ConflictRepository` for the sync-managed entities — the exact same classes the web app uses, backed by the mobile `StorageAdapter` (SQLite via the storage provider; memory in tests). Outbox coalescing, title/category normalization, cycle and delete guards, workspace scoping, and wiki-link maintenance are therefore shared — nothing is reimplemented on mobile. Reminders additionally pass an optional `ReminderNotificationHooks` implementation (the mobile local-notification coordinator) into the shared repository, so every create/update/complete/delete schedules or cancels a native notification without any mobile-specific repository code.
...
BigMind has a two-layer notification story: an in-app **Notification Center** (workspace-scoped records created/stored locally first and synced through the existing engine — ready for future push delivery) and **native local reminder notifications on mobile** (scheduled directly on the device, fully offline).

### Mobile local notifications

On Android the mobile app schedules reminders with the OS `AlarmManager` through `expo-notifications` — no backend, no push token, no network. A scheduled notification survives app restarts **and device reboots** (the library registers `RECEIVE_BOOT_COMPLETED` + reboot receivers). iOS is supported by the same cross-platform API; Android is the first validated target.

- **Create** a reminder → a local notification is scheduled for `dueAt`.
- **Edit** → the notification is rescheduled in place (same identifier).
- **Complete** → the pending notification is canceled.
- **Delete** → the pending notification is canceled.
- **Sync pulls** (reminders changed on another device) → `reconcile()` converges the OS-scheduled set with the local store after every sync pass and on app start.
- The native scheduler is behind a `NotificationScheduler` platform abstraction (`ExpoNotificationScheduler` on device, in-memory in tests), coordinated by `ReminderNotificationService` through the shared repository's `ReminderNotificationHooks`. See [docs/mobile-notifications.md](docs/mobile-notifications.md).
...
- [Mobile Reminders](docs/mobile-reminders.md): Reminder/Agenda architecture on mobile — screens, agenda grouping, reminder synchronization, workspace integration, and performance.
- [Mobile Notifications](docs/mobile-notifications.md): Offline local reminder notifications — platform scheduler abstraction, repository integration, persistence across app restarts/reboots, and sync reconciliation.
...
The **mobile app** currently provides the application shell (React Navigation tabs: Home, Notes, Categories, Reminders, Settings), a full **login/register auth flow** (shared zod-validated contracts, tokens in Expo SecureStore), the **notes and categories experience** (lists and detail screens driven by the shared `@bigmind/features` repositories), the **reminders experience** (agenda grouped Today/Tomorrow/Upcoming/Completed, local search, detail, create/edit form with due date + linked note, completion toggling, confirmed deletion — all offline-first over the shared `RemindersRepository`, see [docs/mobile-reminders.md](docs/mobile-reminders.md)), **offline local reminder notifications** (native Android scheduling through `expo-notifications`; schedule/reschedule/cancel/complete + sync reconciliation, see [docs/mobile-notifications.md](docs/mobile-notifications.md)), shared-library wiring (domain rules, zod contracts, auth state machine, SQLite/`SqliteStorageAdapter` storage), the **native Markdown editor** (Option B: `MarkdownEditView` + shared `@bigmind/markdown` preview — see [docs/mobile-editor.md](docs/mobile-editor.md)), the **workspace experience** (see [docs/mobile-workspaces.md](docs/mobile-workspaces.md)), and the **shared sync engine active on sign-in** (`SyncActivator`: initial pull + AppState/NetInfo background sync, see `apps/mobile/src/sync/`), plus a CI-safe Android bundle build.
